...
task :ocra, :version do |t, args|
  args.with_defaults(:version => '')
  system("ocra --icon './assets/img/icons/parchis.ico' --gemfile Gemfile --add-all-core --gem-full --dll ruby_builtin_dlls/zlib1.dll --dll ruby_builtin_dlls/libssp-0.dll --dll ruby_builtin_dlls/libgmp-10.dll --dll ruby_builtin_dlls/libffi-7.dll --dll ruby_builtin_dlls/libgcc_s_seh-1.dll --dll ruby_builtin_dlls/libssl-1_1-x64.dll --dll ruby_builtin_dlls/libwinpthread-1.dll --dll ruby_builtin_dlls/libyaml-0-2.dll --dll ruby_builtin_dlls/libcrypto-1_1-x64.dll --output 'releases/parchis#{args[:version].!=('') ? "_#{args[:version]}" : ''}.exe' './bin/parchis' './assets/**/*' './lib/**/*'")
end
