...
=end
        ...
        if(button_down?(Gosu::KB_ESCAPE) && (button_down?(Gosu::KB_LEFT_SHIFT) || button_down?(Gosu::KB_RIGHT_SHIFT)))
          ...
          if(player_in_turn.can_roll_dice?)
            ...
            if(button_down?(Gosu::KB_SPACE))
              ...
              @game_state_updater.event_processed(event_id: HTTPClient.post_dice_rolled(result: @dice.roll()))
            ...
            if(player_in_turn.can_move_a? && button_down?(Gosu::KB_A))
              @game_state_updater.event_processed(event_id: HTTPClient.post_token_moved(match_id: @match_id, token_color: player_in_turn.color, token_label: 'A', cells_to_move: @dice.last_roll, end_of_turn: !judge_second_dice_cast(player_in_turn)))
  ...
  def initialize_phase_3(started_by_other: false)
    ...
    @game_state_updater = GameStateUpdater.new(match_id: @match_id, player_id: @player_id, board: @board, dice: @dice, players: @players)
    # widgets
